...
if git status | grep 'not staged' > /dev/null 2>&1; then
    ...
    source venv/bin/activate && HIP=1 FORWARD_ONLY=1 python3 t.py
    exit
EOF
...
echo "remu:"
HIP=1 HIPCPU=1 REMU_DEBUG=1 FORWARD_ONLY=1 python3 ~/code/tinygrad/tinygrad/t.py

echo "tinygrd:"
METAL=1 FORWARD_ONLY=1 python3 ~/code/tinygrad/tinygrad/t.py
