...
mod test_rdna3 {
    ...
    fn have_llvm_tools() -> bool {
        Command::new("llvm-mc").arg("--version").stdout(Stdio::null()).stderr(Stdio::null()).status().is_ok()
            && Command::new("llvm-objdump").arg("--version").stdout(Stdio::null()).stderr(Stdio::null()).status().is_ok()
    }
    ...
    fn test_decode_smem() {
        if !have_llvm_tools() {
            eprintln!("skipping rdna3 llvm-backed test: llvm-mc/llvm-objdump not found");
            return;
        }
