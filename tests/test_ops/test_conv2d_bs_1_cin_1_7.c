...
#define NAN (__builtin_nanf(""))
  ...
  extern "C" __global__
  void __launch_bounds__ (1, 1) r_6_10_3_2_5(float* data0, const float* data1, const float* data2) {
  ...
  for (int ridx0 = 0; ridx0 < 2; ridx0++) {
    for (int ridx1 = 0; ridx1 < 5; ridx1++) {
      ...
      float val1 = *(data2+(gidx0*10)+(ridx0*5)+ridx1);
      acc0 = ((val0*val1)+acc0);
    }
  }
  float alu0 = max(acc0,0.0f);
  *(data0+(gidx0*30)+(gidx1*3)+gidx2) = alu0;
}