...
#define NAN (__builtin_nanf(""))
  ...
  extern "C" __global__
  void __launch_bounds__ (1, 1) r_3_2_7_5(float* data0, const float* data1, const float* data2) {
  ...
  for (int ridx0 = 0; ridx0 < 5; ridx0++) {
    ...
    float val1 = *(data2+(gidx0*10)+(gidx1*5)+ridx0);
    acc0 = ((val0*val1)+acc0);
  }
  float alu0 = max(acc0,0.0f);
  *(data0+(gidx0*14)+(gidx1*7)+gidx2) = alu0;
}