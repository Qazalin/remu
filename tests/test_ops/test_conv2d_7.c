#include <hip/hip_common.h>
#define INFINITY (__builtin_inff())
#define NAN (__builtin_nanf(""))
  typedef float float8 __attribute__((ext_vector_type(8)));
  __device__ float8 make_float8(float x, float y, float z, float w, float a, float b, float c, float d) { return {x, y, z, w, a, b, c, d}; }
  extern "C" __global__
  void __launch_bounds__ (1, 1) r_6_10_3_3_2_5(float* data0, const float* data1, const float* data2) {
  int gidx0 = blockIdx.z; /* 6 */
  int gidx1 = blockIdx.y; /* 10 */
  int gidx2 = blockIdx.x; /* 3 */
  float acc0 = 0.0f;
  int alu0 = (gidx0*30);
  for (int ridx0 = 0; ridx0 < 3; ridx0++) {
    for (int ridx1 = 0; ridx1 < 2; ridx1++) {
      for (int ridx2 = 0; ridx2 < 5; ridx2++) {
        float val0 = *(data1+(gidx1*7)+gidx2+(ridx0*77)+(ridx1*7)+ridx2);
        float val1 = *(data2+alu0+(ridx0*10)+(ridx1*5)+ridx2);
        acc0 = ((val0*val1)+acc0);
      }
    }
  }
  float alu1 = max(acc0,0.0f);
  *(data0+alu0+(gidx1*3)+gidx2) = alu1;
}