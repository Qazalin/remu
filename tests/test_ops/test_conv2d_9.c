...
#define NAN (__builtin_nanf(""))
  ...
  extern "C" __global__
  void __launch_bounds__ (1, 1) r_6_9_6_3_3_2(float* data0, const float* data1, const float* data2) {
  ...
  for (int ridx0 = 0; ridx0 < 3; ridx0++) {
    for (int ridx1 = 0; ridx1 < 3; ridx1++) {
      for (int ridx2 = 0; ridx2 < 2; ridx2++) {
        ...
        float val1 = *(data2+(gidx0*18)+(ridx0*6)+(ridx1*2)+ridx2);
        acc0 = ((val0*val1)+acc0);
      }
    }
  }
  float alu0 = max(acc0,0.0f);
  *(data0+(gidx0*54)+(gidx1*6)+gidx2) = alu0;
}