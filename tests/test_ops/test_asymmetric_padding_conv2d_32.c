...
#define NAN (__builtin_nanf(""))
  ...
  extern "C" __global__
  void __launch_bounds__ (1, 1) r_6_5_3_3(float* data0, const float* data1) {
  ...
  for (int ridx0 = 0; ridx0 < 3; ridx0++) {
    ...
    for (int ridx1 = 0; ridx1 < 3; ridx1++) {
      ...
      int alu4 = (alu3%7);
      float val0 = (((alu1<12)*(alu0<14)*(alu2<4)*(alu4<5)))?(*(data1+(alu2*4)+((alu1/6)%2)+(alu4*16)+(((alu3/7)%2)*2))):0.0f;
      acc0 = (val0+acc0);
    }
  }
